...
# AI 编程范式演进：从代码补全到 Vibe Coding、Spec Coding
...
1. **Plan Mode**：先规划再执行。按 Shift+Tab 切换到规划模式，Agent 会先研究代码库、询问你一些澄清问题、创建详细计划，等你批准后再开始写代码。
...
另外像是 Opencode 等开源 Coding 工具也都是完整支持 plan 模式的。并且越来越多的工具、Agent 也纷纷加入了 plan 阵营。
...
- 想法靠谱了，将想法转换为可落地的规范
- 核心业务逻辑自己把关
- 单元测试以及 code review 作为最后的兜底必不可少
...
尤其是 Vibe Coding 生成的代码，你必须审查。Code review 作为最后的兜底是最重要的一步。
